...
module DMEM(                    //DMEM根据性能考量，设计成异步读取数据，同步写入数据的形式
    ...
    input [31:0] dm_addr,        //7位地址，要读取/写入的地址
    input [31:0] dm_data_in,    //写入时要写入的数据
    output [31:0] dm_data_out   //读取时读取到的数据
    );

reg [31:0] dmem [1023:0];//DMEM区域

assign dm_data_out = (dm_ena && dm_r) ? 
...
always @(posedge dm_clk)//时钟上升沿写入数据
begin
    if(dm_ena && dm_w)//必须是使能端开启、写指令有效且读指令无效时，才向寄存器中写入数据
    begin
